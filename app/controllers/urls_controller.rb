class UrlsController < ApplicationController
  ...
  def redirect
    ...
    if url
      ...
      respond_to do |format|
        format.html { redirect_to urls_path, notice: "Url not found." }
        format.json { render json: { error: "Url not found" }, status: :not_found }
        format.plain { render plain: "Url not found", status: :not_found }
      end
    end
  end
  ...
  private
    ...
    def url_params
      params.expect(url: [ :original, :shotened, :string ])
    end
end
